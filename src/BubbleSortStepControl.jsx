...
function BubbleSortStepControl() {
  ...
  const reset = useCallback(() => {
    const newBubbles = Array.from({ length: n }, (_, i) => ({
      ...
      position: i * 60,
      color: '#6b73ff',
      sorted: false  // Track if this bubble is in final sorted position
    }));
  ...
  const nextStep = useCallback(() => {
    ...
    // Get bubbles at current positions
    const pos1 = j * 60;
    const pos2 = (j + 1) * 60;
    ...
    setTimeout(() => {
      ...
      if (bubble1.value > bubble2.value) {
        ...
        // No swap - reset comparison colors
        newBubbles = newBubbles.map(b => {
          if (b.position === pos1 || b.position === pos2) {
      ...
      setBubbles(newBubbles);
      // STEP 3: Mark CURRENT PASS complete (rightmost bubble goes blue)
      setTimeout(() => {
        const sortedPosition = (n - i - 1) * 60;
        setBubbles(prev => prev.map(b => {
          // ✅ FIXED: Mark bubble at sorted position as blue + sorted
          if (b.position === sortedPosition && !b.sorted) {
            return { ...b, color: '#1e90ff', sorted: false};
          }
          // Reset temp colors
          if ((b.color === '#ff4757' || b.color === '#2ed573') && !b.sorted) {
            return { ...b, color: '#6b73ff', scale: 1 };
          }
          return b;
        }));
  ...
  return (
    <div className="bubble-container">
      <h2>✅ FIXED Blue Sort - Step Control</h2>
      ...
      <div className="status">
        <p>🔴 <strong>Red + Pulsing</strong> = Comparing</p>
        <p>🟢 <strong>Green Flash</strong> = Just Swapped</p>
        <p>🔵 <strong>Blue</strong> = <strong>PERMANENTLY SORTED</strong> ✅</p>
        <p>🟣 <strong>Purple</strong> = Still needs sorting</p>
      </div>
    </div>
  );
}

export default BubbleSortStepControl;